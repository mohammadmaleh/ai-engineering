...
- **ENG-12 follow-up (next-day, in review):** honest schema review flagged that the merged ERD was *design-level, not production*. Added a **"Constraints & types"** section — `UNIQUE(user_id, organization_id)` on memberships, enum/`CHECK` on role/status/doc_type/urgency, **`timestamptz` not naive `timestamp`**, `patients.patient_ref`, append-only trails, polymorphic `audit_log.entity_id` (no FK). Taught **constraints belong in the DB, not app code** (a constraint can't be violated; app checks get forgotten/raced). ⚠️ **Pushed as branch `ENG-12-constraints` — PR OPEN, NOT yet merged.** If not merged by next session: merge `main...ENG-12-constraints` (set squash title), then it's done. `notes/erd.md` updated with the lesson.
