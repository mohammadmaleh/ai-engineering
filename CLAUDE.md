...
- [~] **M0 — Re-entry** — backend RUNS again (venv + all deps + `.env` with all 5 keys OK, app imports, `/docs` loads, migrated). **Only the end-to-end chat smoke test remains** (upload 1-page fixture → ask a question) to fully close it.
- [x] **M1 — System design package** — **DONE 2026-07-15.** ENG-10→ENG-17 all written, merged to `main`, and Done in Linear. 8 cross-referenced docs in `meddocs/docs/` + `PRODUCT.md` + `notes/` cheat sheets.
...
- **🟢 JWT MECHANISM — PASSED on the 4th attempt** (end of session, cold): *"the server takes the env key, **generates a signature again**, then **compares the generated signature to the signature received**."* Both sides named, key as ingredient. **Re-confirm once more cold next session, then retire it.** (Sharpening given: recomputed *from the incoming header+payload*. Also taught: `exp` lives in the **payload**, so the signature covers it — you cannot extend your own token.)
- **(superseded) JWT had failed a THIRD time earlier in this session** — He now says *"we check the signature **against the key** in the env var."* Still wrong. **Both sides of the comparison are signatures**: the server compares **the signature in the token** vs **a signature it recomputes** from the incoming `header.payload` using `SECRET_KEY`. **The key is an INGREDIENT, not a side of the comparison** (flour, not cake). Ask it cold, every session, until automatic.
...
- **🔴 START EVERY SESSION WITH 3 COLD QUESTIONS PULLED FROM `notes/` — ROTATING.** **He asked for this himself** (2026-07-12): *"you keep asking me about jwt, but you don't question me about the notes."* He is right — drilling one item while 20 others sit unexamined is the same "reread ≠ learn" trap. Anything in `notes/` is fair game: `rbac.md`, `erd.md`, `architecture.md`, `auth-jwt.md`, `rag-ai.md`, `fastapi.md`, `postgres-sqlalchemy.md`. **Vary them. He remembers what he is made to retrieve.**
...
## Last Session (2026-07-15) — M1 CONTENT COMPLETE (ENG-14 → ENG-17, 4 PRs open)

- **Shipped all four remaining M1 tickets in one long session — and he MERGED all four himself with clean squash titles. M1 is DONE.** #29 ENG-14 (workflow), #30 ENG-15 (api-conventions), #31 ENG-16 (ADR-001–005), #32 ENG-17 (wireframes) — all on `main`; all four flipped to **Done** in Linear (board matches git). No open PRs. Notes written for each: `notes/state-machines.md`, `notes/rest-conventions.md`, `notes/architecture-decisions.md`. **The squash-title habit is fully stuck (4 clean merges).**
- **M0 re-entry started + backend confirmed running:** `backend-on` (zsh function: `sudo systemctl start postgresql` → `cd backend` → `source venv/bin/activate`; Postgres/Docker are off-at-boot on purpose) then `uvicorn main:app --reload` (or the `fastapi-dev` alias). Python 3.12.6, all deps present, `.env` has DATABASE_URL/SECRET_KEY/OPENAI/PINECONE/GROQ, app imports clean, `/docs` loads. **NOT yet proven end-to-end** — the upload→embed→chat smoke test is the M0 finish line and the M2 prerequisite.
- **ENG-14 (`workflow.md`):** he led the design. Landed: 8 states (added `awaiting_second_opinion`), 12 guarded transitions, the accountability invariant (`start_review`/`approve`/`reject` locked to `assigned_user_id = caller`), continuity = **reassignment not bypass**, terminal states + "a redo is a NEW document entity" (he invented this himself), service-account actor for AI transitions (**not** impersonation — corrected). Then I ran a **design audit** and found 4 real holes; he had me fix all: (1) urgent-doc-in-pool had no SLA → **two SLA clocks** (pool@triaged, review@assigned); (2)+(4) added a **second axis** `pipeline_status` (pending|processing|done|failed) — which **dissolved** the hidden-triage-state problem ("needs human triage" = `received`+`pipeline_status=done`, queryable); (3) added **unassign** (`assigned→triaged`). Synced the `documents.status` enum across `erd.md` + `PRODUCT.md`.
- **ENG-15 (`api-conventions.md`):** /api/v1, one error envelope `{error:{code,message,details}}`, **status(coarse)-vs-code(specific)**, offset-now/cursor-later, transitions as action endpoints not `PATCH status`.
- **ENG-16 (5 ADRs):** monolith / shared-DB-tenancy+RLS / JWT+RBAC / build-flags / object-storage+presigned-URLs. **Resolved 2 architecture.md findings** (A: authN≠authz numbering; B: presigned-vs-proxy) inside ADR-003 & ADR-005. **Findings C (observability missing from C4 diagrams) + D (Next.js "runs in browser" is wrong) are still open** — pinned as a comment on ENG-16, to fix when architecture.md is next touched.
- **⚠️ ENG-17 honesty note:** its DoD wants **FigJam frames**; I delivered **markdown ASCII wireframes** (`docs/wireframes.md`) instead, with his up-front OK, to finish M1 in-repo. If he wants a portfolio-grade visual, real Figma is an **M5** task. Don't let it be recorded as "FigJam done."

### Re-test / drill next session (rotating cold questions)

- **✅ JWT MECHANISM — PASSED cold AGAIN (5th pass).** *"jose gets the signature part, generates a new signature from the key + payload, compares the two."* **RETIRED. Do not ask again.**
- **🔴 HTTP STATUS CODES are shaky and interview-critical.** In one session he forgot **409** three times, guessed `422`, then said `402`=forbidden (402 is Payment Required; 403 is Forbidden). Drill the anchor cold: **four gates → four codes: authN 401 · RBAC 403 · tenancy 404 · guard 409.** (Memory saved: `coaching_http_status_codes`. Card: `notes/state-machines.md` §0, `notes/rest-conventions.md` §1.)
- **🔴 `document_events` vs `audit_log` — still conflated** (said "audit_log" when the answer was `document_events` for "who touched THIS doc"). Keep drilling: *this doc's timeline* → `document_events`; *org-wide who-did-what incl. reads, for the auditor* → `audit_log`.
- **🔴 ROOT PATTERN persists — one-sided answers** (names the label, skips the mechanism / the other side of a comparison). "A comparison has TWO sides — name both."

### How he worked this session (important shift)

- **He disengaged from deep defending and pushed to *finish tickets*.** Said: *"I won't defend anything, I'm just gonna talk about it — no interviewer will discuss this, they'll assume it's AI-generated. The goal is for me to learn."* I reframed (didn't just accept): interviewers won't read the doc but **will** ask *"why a monolith / how do you isolate tenants / walk me through auth"* = the ADR topics; and for *learning*, defending beats passive talk-through (the JWT reread-trap proof). **He half-accepted.** Net: I switched to **"I draft, walk him through the essence, he reacts"** for ENG-15/16/17 rather than hostile drill. See memory `coaching_defend_vs_talk`. Expect this tension again — hold the line that active retrieval is the method, but meet his energy.
- **He values throughput when tired** — keep messages SHORT (still true), lead with the concrete answer to his direct question, don't over-Socratic when he's spent.
- **Good instinct banked:** "the stack follows the workflow" (deferred tech/ADR decisions correctly) and "is the project solid, are there loops?" (asked for a *design audit*, not self-testing) — both senior moves; tell him so.

### ▶ NEXT SESSION — M2 KICKOFF (start here; M1 is done, don't re-do it)

**Session-start ritual first** (per "How to work with him"): 3 cold rotating questions from `notes/`. Good candidates now — **the 5 ADR soundbites** (this closes the one M1 gap: the ADRs were written by Claude but never *defended* — quizzing them cold IS the deferred ENG-16 challenge round), **HTTP codes** (four gates→four codes: 401/403/404/409 — he forgot 409 3× last session), and **`document_events` vs `audit_log`** (still conflates them).

**Then M2 — Foundation (PLAN.md §6 "M2", 3 weeks). The order that matters:**
1. **CLOSE M0 FIRST — one 5-min smoke test.** `backend-on` → `uvicorn main:app --reload` → in `/docs`: register → login → upload a **1-page** fixture → chat with it. Confirms the pipeline actually answers before he changes it. If keys are stale / Pinecone index gone, fix that first. **Do not start D1 until a chat answer comes back locally.**
2. **Then the defect audit D1–D8** (`PLAN.md` §4, verified against code, with `file:line`). **Start with D1 — fake citation page numbers** (`document_processor.py:50` computes them arithmetically while real page numbers from `:20-25` are thrown away). It's the perfect opener: **he already designed the fix in M1** (per-page chunking, real metadata). Each defect = its own PR with a description naming the defect + the concept it teaches.
3. Then the rest of M2: `organizations` + `memberships` + **tenancy-isolation test written FIRST**; RBAC `require_permission` dependency; refresh-token rotation + invitations; object storage for PDFs; `/api/v1` + error envelope (per `api-conventions.md`); fixtures + seeder; **first CI (ruff+black+mypy+pytest) + first deploy** (Render + Neon).

**Critical M2 facts for the next AI:**
- **The current `backend/app/models.py` is the OLD single-user schema** (User, Document, ChatSession, ChatMessage). M2 **rebuilds** it to the multi-tenant `erd.md` (12 tables, uuid PKs, `organization_id` everywhere). Do NOT reuse the old schema.
- **He writes every line of code** (Python 1/10 — go slow, one concept per message, explain then build). Claude reviews/challenges/quizzes; only writes code when explicitly asked. M2 CANNOT be drafted for him the way the M1 docs were.
- **Cost rules hold:** Groq for chat, `text-embedding-3-small` for embeddings, **1–2 page fixtures only**, mock all LLM/vector calls in pytest (no API calls in test loops).
- **Repo layout trap:** M2 code → the **inner** `meddocs/` repo (branch, PR, merge there, `main` protected — set squash title at merge). `notes/*.md` → the **outer** repo, committed separately. Always `git -C <path>`.
- **Working-method note (2026-07-15):** he pushed to *finish tickets* over deep drilling and said "no interviewer will discuss the docs, they'll assume AI wrote them" — reframed (they ask the ADR *topics*; defending = the learning method). See memory `coaching_defend_vs_talk`. Meet his energy but keep active retrieval alive.

**Small open items (not blockers):** architecture.md findings **C** (Langfuse/Sentry missing from C4 diagrams) + **D** (Next.js "runs in browser" is wrong) — pinned as a comment on ENG-16, fix when architecture is next touched. `PLAN.md` §131–134 state-machine sketch is stale (`NEEDS_SECOND_OPINION` → `awaiting_second_opinion`; `PROCESSING`/`FAILED` belong on the separate `pipeline_status` axis) — 2-min outer-repo fix, not yet done. ENG-17 wireframes are markdown (not FigJam) by agreement — real Figma is M5.
