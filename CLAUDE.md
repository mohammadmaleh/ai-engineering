...
- **Date:** 2026-07-11
- **Phase / topic covered:** **M1 kickoff — product definition (ENG-10).** Wore the product-manager hat. (Carryover context still valid: PLAN.md v3, FleetPulse is his 2nd portfolio piece, don't mention Talpa, Claude writes the `notes/` files now.)
- **What we did:** **Rust-check redrill** of the two decayed concepts. JWT statelessness **landed** this time after teaching (client stores the token; server *recomputes the signature* with `SECRET_KEY`, no DB lookup; writes record themselves, reads don't). Cosine similarity **still shaky** — see below. **Created all 8 M1 tickets in Linear** (ENG-10→17) under milestone *"M1 — System design package"* (target Jul 21); ENG-11→17 are `blockedBy` ENG-10. **Built `meddocs/PRODUCT.md` (ENG-10) collaboratively — he made every call, I challenged:** 4 personas, 3 journeys (intake&triage / review&sign-off / oversight&audit) with document states, **corrected the auditor role** (read-only compliance, NOT an approver — separation of duties), taught **system boundaries** (MedDocs owns the *document* slice, not the hospital — appointments/exams/billing are outside; wrote explicit **non-goals**), **scope discipline** (deferred vacation/capacity/offboarding to a LATER list), added **patients as first-class** (org→patient→documents, fake data), and the **audit-log concept** (writes self-record, reads vanish unless logged at event time — same "record-when-it-happens" pattern as flag-at-upload). **Shut down hard scope creep** — he wanted to build a "full clinic system"; reframed as a non-goal with the interview framing ("deliberate scope, shipped a defensible slice"). **Upgraded `/commit`** (`.claude/commands/commit.md`): now detects the right repo (outer `ai-engineer` vs the **nested `meddocs` repo** — they're separate repos with separate GitHub remotes), reads the ticket number, and creates/uses an `ENG-<n>-<slug>` branch so Linear links it. He committed that upgrade himself (outer repo `ef2bd9a`).
- **Where we stopped:** **ENG-10 done & pushed.** MedDocs branch `ENG-10-product-definition` (commit `3cffa8c`), Linear-linked, ticket moved to **In Progress**. Both repos clean. **PR not yet opened** (M1 DoD wants "merged via PR" — open `ENG-10-product-definition` → `main` and merge when ready, then flip ENG-10 to Done).
- **Next task:** **ENG-11 — `meddocs/docs/architecture.md`** (C4 context + container diagrams, tech-choice table). Then ENG-12 `erd.md` (design the *new* multi-tenant schema — current `models.py` is the OLD single-user one), ENG-13 permissions, ENG-14 workflow, ENG-15 api-conventions, ENG-16 ADRs (the challenge rounds), ENG-17 wireframes. He leads each doc; I attack, he defends.
- **Things Mohamad was shaky on — re-test next session:** **COSINE SIMILARITY — the *term* decayed AGAIN** (3rd time). He has the intuition ("the arrow thing," direction) but blanked on the name at session end. Drill the exact sentence: *"cosine similarity = the angle between two vectors; small angle = same meaning."* JWT statelessness was better this time but re-confirm once. He **undervalues non-coding work** ("felt good but not productive") — reframe design/scoping as senior work, not filler. Older carryover: why keyword flagging runs at upload not chat; `enumerate()` order (index first); `+=` vs `append`; `# type: ignore`.
