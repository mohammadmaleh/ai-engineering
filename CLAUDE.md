...
- **Shipped all four remaining M1 tickets in one long session. M1 is now 8/8 in content.** Four PRs open, **not yet merged** — he must merge them: **#29 ENG-14** (workflow), **#30 ENG-15** (api-conventions), **#31 ENG-16** (ADR-001–005), **#32 ENG-17** (wireframes). Each PR body carries its exact **squash-merge title**. Linear ENG-14/15/16/17 all set to **In Progress** (correct — git not yet the source of truth). **First thing next session: verify he merged all four + flipped them to Done (board-drift check, 4th time's the pattern).** Notes written for each: `notes/state-machines.md`, `notes/rest-conventions.md`, `notes/architecture-decisions.md`.
...
- **Verify the 4 merges first** (board-drift check). Then **M1 is DONE → M2 begins** (backend foundation: fix defects D1–D8, tenancy + RBAC in code, refresh tokens, object storage, first CI + deploy). **M2 is where he writes real code again** — Python 1/10, go slow, he writes every line. Also: architecture.md findings C+D still open; `PLAN.md` §131–134 state-machine sketch is stale (offered a fix, not yet done — outer repo).
