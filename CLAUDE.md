...
- **Date:** 2026-07-11
- **Phase / topic covered:** **M0 re-entry — executed and completed.** (Prior context: PLAN.md v3 finalized, Claude tooling researched → `notes/claude-tooling.md`, Talpa references removed after a rejection — do not bring it up. Mohamad also has a second portfolio piece, FleetPulse, a React/TS fleet-ops dashboard.)
- **What we did:** Booted the backend — deleted a stray `.venv` (kept one clean `venv`), started native Postgres (`sudo service postgresql start`), migrations already at head (`7140bd44c92c`), server runs on `:8000`. Ran the **full smoke test** via API (register → login → upload a generated 1-page fixture PDF → chat): RAG works end-to-end and answered correctly — and the raw stream exposed **defect D3 (broken SSE framing) live** (bare tokens then a jammed-on `data:` event). **Rust-check out loud**, corrected two *decayed* misconceptions: (1) Pinecone matches by **cosine similarity** = angle between vectors = *meaning*, not shared words; (2) JWT is **stateless** — verified by signature with `SECRET_KEY`, NOT compared to a token stored in the DB. Postgres **client-server vs SQLite** understood (4/10 → 8/10). New arrangement: **Claude writes the `notes/` files now** (Mohamad doesn't want to write much) — compensate with out-loud drilling. Updated `notes/rag-ai.md` (cosine) and `notes/auth-jwt.md` (stateless). Built a **`/commit` custom slash command** (`.claude/commands/commit.md`): reviews the diff via `/code-review`, raises flags, does a secrets sweep, drafts a Conventional Commit message, then **STOPS for approval before pushing** (human-in-the-loop by design — don't make it auto-push while he's learning).
- **Where we stopped:** **M0 DONE and committed** (commit `02e3c8c`, pushed, `main` clean and in sync with origin). Backend runs and answers a chat question locally.
- **Next task:** **M1 — system design package** (`PLAN.md` §6 → M1): `docs/architecture.md`, `erd.md`, `permissions.md`, `workflow.md`, `api-conventions.md`, ADR-001–005, `PRODUCT.md`, + FigJam wireframes for the inbox/queues and document-workspace screens. Break M1 into tickets at the start.
- **Things Mohamad was shaky on — re-test next session:** **cosine similarity** and **JWT statelessness** — he explained BOTH wrong out loud today despite them being correct in his own April notes, so they decay fast; re-drill both. He resists out-loud explanation and tried to skip the smoke test and rust-check — hold the line, it worked. Older carryover: why keyword flagging runs at upload not chat; `enumerate()` order (index first); `+=` vs `append`; `# type: ignore` for untyped libs.
