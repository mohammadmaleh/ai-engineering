...
- **ENG-12 STARTED (`erd.md`):** he designed the schema skeleton by reasoning, DB design being his self-declared weakest point. Built the core entity list, then landed two marquee concepts — see shaky/wins below. **First-draft `meddocs/docs/erd.md` written** (branch `ENG-12-erd`, off `origin/main`, **uncommitted**): 7 core tables — `organizations, users, memberships, patients, documents, document_events, audit_log` — `organization_id` on every one **except `users`** (global identity, scoped via memberships). Plus `notes/erd.md` cheat sheet.
- **Next task: finish ENG-12.** Next session (he's fresh): **he attacks the draft**, adds **secondary tables** (`comments, lab_values, notifications`, chat tables), decides **PK type — `int` vs `uuid`** (multi-tenant security angle → ADR), reviews cardinality + indexes, then `/commit` on `ENG-12-erd` → PR → **SET THE SQUASH TITLE at merge**. Then ENG-13 permissions matrix, ENG-14 workflow state machine, ENG-15 api-conventions, ENG-16 ADRs (ADR-001 monolith, 002 tenancy, 003 authz, 004 flags, 005 storage), ENG-17 wireframes. He leads each doc; I attack, he defends.
- **Things Mohamad was shaky on — re-test next session:** **JWT MECHANISM** — he thinks the signature lives in the env var. Drill: *"env holds the `SECRET_KEY`; the server recomputes the signature and compares."* **Cosine** finally passed but re-confirm once cold. He **reaches for vendor names too early** (S3, Gmail) — reinforce "capability at L1, vendor in the ADR." He **still undervalues design/non-coding work** — keep reframing it as senior work. **PROCESS habit to drill: set the squash-merge title at merge time** — ENG-10 landed clean but ENG-11 shipped with a wrong squash message that can't be fixed on protected `main`. **DB WINS to reinforce (his weakest area — big progress):** he LANDED the **`memberships` join table** cold — explained M:N (users↔orgs) → join table → **role lives on the link, not on `users`** — near-verbatim, unprompted. Also taught the **`document_events` vs `audit_log` two-trails** split (a *read* hits `audit_log` only — no state change → nothing in `document_events`). Re-confirm both next session; he's shaky enough on DB that these need a cold re-test. Older carryover: why keyword flagging runs at upload not chat; `enumerate()` order; `+=` vs `append`; `# type: ignore`.
